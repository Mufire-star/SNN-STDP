...
int main()
{
    ...
    const int pred_baseline = run_infer(in_stream, out_stream, images[0]);

    std::vector<std::vector<unsigned char>> train_imgs(images.begin() + 1, images.begin() + 1 + NUM_TRAIN_IMG);
    const int pred_after_train = run_train_then_infer(in_stream, out_stream, train_imgs, images[0]);
